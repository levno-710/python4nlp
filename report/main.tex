...
examples onward, it outperforms TF--IDF in all three test runs. On validation,
this consistency begins at 20 examples. The largest test gap occurs at 50
...
the threshold for a consistent advantage across both splits,
although computational cost and low-data instability remain substantial.
\end{abstract}
...
three. 20 examples per class therefore is the threshold at
which the advantage is consistent.
...
For this dataset and training procedure, 20 examples per class are therefore a
conservative threshold for a consistent advantage across both splits, while
roughly 50 are needed for a clear gain.
