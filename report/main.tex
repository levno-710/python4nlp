...
\begin{abstract}
Pretrained language models are often recommended when labelled data is scarce. I test when a
pretrained DistilBERT classifier begins to outperform a simpler lexical model on
four-class emotion classification. From GoEmotions, I use English Reddit
comments with exactly one of four labels: joy, anger, sadness, or fear. I compare
DistilBERT with TF--IDF logistic regression using balanced, nested training sets
containing 5 to 200 examples per class. Each condition is repeated with three
random seeds and evaluated on the original validation and test splits using
...
Face Datasets library \citep{lhoest2021datasets}. The dataset is released under
...
per class, TF--IDF has the higher macro-F1. The mean scores are nearly equal at
10 examples, but DistilBERT's standard deviation is four times larger. Between
10 and 20 examples, its mean is higher but at least one seed still performs
worse than the corresponding TF--IDF run. At 25 examples, DistilBERT wins for
all three paired seeds. Its mean advantage is .165 macro-F1. The largest mean
gap, .207, occurs at 50 examples per class.
...
\begin{table}[ht]
  ...
  \begin{tabular}{rrrrrr}
    ...
    \midrule
      5 &  20 & \textbf{.374 $\pm$ .024} & .383 & .310 $\pm$ .028 & .396 \\
     10 &  40 & .410 $\pm$ .029 & .411 & \textbf{.417 $\pm$ .118} & .461 \\
     15 &  60 & .445 $\pm$ .026 & .450 & \textbf{.511 $\pm$ .116} & .545 \\
     20 &  80 & .481 $\pm$ .021 & .484 & \textbf{.586 $\pm$ .113} & .605 \\
     25 & 100 & .511 $\pm$ .014 & .517 & \textbf{.675 $\pm$ .053} & .690 \\
     50 & 200 & .615 $\pm$ .024 & .616 & \textbf{.822 $\pm$ .025} & .826 \\
    100 & 400 & .686 $\pm$ .026 & .688 & \textbf{.855 $\pm$ .013} & .859 \\
    200 & 800 & .734 $\pm$ .011 & .734 & \textbf{.867 $\pm$ .016} & .868 \\
    \bottomrule
  \end{tabular}
\end{table}
...
macro-F1 scores are .621, .676, and .460. The curve begins
to flatten after 50 examples: its mean improves by .033 from 50 to 100 and only
.011 from 100 to 200. TF--IDF continues to improve, so the absolute gap narrows
after its maximum at 50 examples.

At 200 examples per class, mean measured training
time is 1.44 seconds for TF--IDF and 74.19 seconds for DistilBERT, about 52 times
longer. The corresponding gain is .133 macro-F1.
...
At 200 examples per class, DistilBERT's mean per-class F1 scores are .918 for
joy, .864 for anger, .848 for sadness, and .837 for fear. TF--IDF scores .803,
...
the three runs, are anger predicted as sadness (33 cases), sadness as fear (23),
and anger as fear (18).
...
seed-dependent through 20 examples. At 25 examples per class it beats TF--IDF
for every seed, and at 50 examples the advantage is large: .822 versus .615
macro-F1. Improvements after 100 examples are small. For this dataset and
training procedure, 25 examples per class are therefore enough for a consistent
...
I used OpenAI Codex, to brainstorm the research question, assist
with implementation and debugging and assist with revising the report text.
I remain responsible for the submitted code, analysis, and text.
