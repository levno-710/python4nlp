...
examples onward, it outperforms TF--IDF in all three observed test runs. On
validation, this pattern begins at 20 examples. The largest test gap occurs at 50
examples (.816 versus .615). At 200 examples, DistilBERT reaches .865 macro-F1
but trains about 57 times longer. I therefore identify 20 examples per class as
the first tested size with an advantage in all three runs on both splits,
although three seeds cannot establish a general threshold and computational
cost and low-data instability remain substantial.
\end{abstract}
...
This report aims to answer the question: \emph{At what tested training size does
fine-tuned DistilBERT consistently outperform TF--IDF logistic regression on
...
advantage occurs in every observed paired run. Here, ``consistent'' means higher
macro-F1 in all three paired runs. It does not imply a statistically established 
threshold.
...
the Apache 2.0 licence \citep{demszky2020goemotions}. I keep an example only if it has exactly one label and
...
The baseline is a scikit-learn pipeline \citep{pedregosa2011scikit} consisting
...
\citep{wolf2020transformers,paszke2019pytorch}.
...
test examples during training. All training sizes, hyperparameters, and
evaluation procedures were fixed before inspecting the evaluation results. The
test split provides the primary final comparison.
...
The validation results show the same overall transition but place the
consistent crossover slightly later. At 15 examples per class, DistilBERT wins
two of the three paired validation runs. From 20 examples onward, it wins all
three. Thus, 20 examples per class is the first tested size at which the
advantage occurs in every observed run on both splits.
...
\begin{table}[ht]
  ...
  \begin{tabular}{p{.48\linewidth}lll}
    ...
    \midrule
    ``-9 and I’m out shopping. Sun is out feels great.'' & Joy & Sadness/fear & Joy \\
...
for every test seed but only two of three validation seeds. From 20 examples it
wins every paired run on both splits. At 50 examples the test advantage is
large: .816 versus .615 macro-F1. Improvements after 100 examples are small.
For this dataset, training procedure, and set of three seeds, 20 examples per
class are therefore the first tested size with an advantage in every run on both
splits, while roughly 50 are needed for a clear mean gain. More seeds would be
required to determine whether this crossover generalizes.
